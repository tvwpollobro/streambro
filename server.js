...
const DIRECTORIO_SITIOS = {
    cinecalidad: ['https://www.cinecalidad.gg', 'https://www.cinecalidad.to', 'https://www.cinecalidad.is'],
    cuevana: ['https://cuevana3.ch', 'https://cuevana.biz', 'https://cuevana3.io'],
    pelisplus: ['https://pelisplus.to', 'https://pelisplus.so', 'https://pelisplus.lat'],
    gnula: ['https://gnula.nu', 'https://gnula.se']
};
...
app.get('/', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            #splash { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background-color: #000; z-index: 9999; display: flex; flex-direction: column; justify-content: center; align-items: center; transition: opacity 1.5s ease; }
            ...
            @keyframes latido { 0% { transform: scale(1); } 20% { transform: scale(1.15); } 40% { transform: scale(1); } 60% { transform: scale(1.15); } 100% { transform: scale(1); } }
            ...
            .boton-tv { background-color: #222; color: white; border: 3px solid #444; padding: 25px 30px; font-size: 22px; font-weight: bold; border-radius: 10px; cursor: pointer; width: 220px; transition: 0.2s; text-decoration: none; display: inline-block; box-sizing: border-box; }
            .boton-tv:focus, .boton-tv.focused { border-color: #fff; background-color: #E50914; transform: scale(1.08); outline: none; box-shadow: 0 0 15px #E50914; }
        </style>
    </head>
    <body>
        <div id="splash">
...
app.get('/ver', async (req, res) => {
    ...
    for (const url of dominiosDisponibles) {
        try {
            ...
            console.log("Probando alternativa...");
        }
    }
    if (!htmlLimpio) return res.status(503).send("Ninguna extensión responde.");
